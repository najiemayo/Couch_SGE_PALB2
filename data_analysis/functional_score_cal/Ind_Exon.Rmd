...
exonDataList <- readr::read_table("/research/data/exonDataList.txt")
...
target <- read.csv("/research///chunling_target.csv")
...
ET <- read.csv("/research//chunling_ET.csv") %>% rename(EventType = ET) %>% select(-uid)
...
```{r}
library(readxl)
Varaints_to_be_excluded_when_use_MAVE_internal_control_to_build_model <- read_excel("/research//Varaints to be excluded when use MAVE internal control to build model.xlsx")
