...
exonDataList <- readr::read_table("###BRCA2/data/exonDataList.txt")
...
target <- read.csv("###BRCA2/data/10_2023_curation//chunling_target.csv")
...
ET <- read.csv("###BRCA2/data/10_2023_curation/chunling_ET.csv") %>% rename(EventType = ET) %>% select(-uid)
...
```{r}
library(readxl)
Varaints_to_be_excluded_when_use_MAVE_internal_control_to_build_model <- read_excel("###BRCA2/data/10_2023_curation/Varaints to be excluded when use MAVE internal control to build model.xlsx")
