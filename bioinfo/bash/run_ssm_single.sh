...
GENERATE_SINGLE_JSON="${SCRIPT_DIR}/python/generate_json_singleread.py"
FASTQ2BAM_WDL="${SCRIPT_DIR}/wdl/workflows/SingleReadToSam.wdl"
...
function generate_json_cfg {
    echo ${CUTADAPT_PARAM}
    cmd="${PYTHON} ${GENERATE_SINGLE_JSON} -f ${FASTQ_DIR}  -o ${OUT_DIR} -c '${CUTADAPT_PARAM}' -r ${REFERENCE} -j ${SCRIPT_DIR}/wdl/SingleReadToSam.json"
    echo ${cmd}
    eval ${cmd}
